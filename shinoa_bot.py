...
async def stats(interaction: discord.Interaction):
    ...
    embed.set_footer(text="Model: gemini-1.5-flash | Auto-delete: 30 days")
    await interaction.response.send_message(embed=embed, ephemeral=True)
...
async def generate_response(uid: int, msg: str) -> str:
    ...
    if uid not in chat_sessions:
        try:
            # CORRECT: gemini-1.5-flash — NO "models/" prefix
            ...
            print(f"[MODEL] Created session for user {uid} using gemini-1.5-flash")
        except Exception as e:
